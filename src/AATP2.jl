function solve_AATP2_exact(p0::AbstractArray{<:Real},
...
function find_λ_AATP2(μ, s, C2::Integer)
    i, j, d  = 2, 1, 1
    λ, λ_old = s[1], 0
    g, g_old = -C2*μ, -C2*μ
...
function find_μ_AATP2(μ, s, p0, q0, C1::Real, C2::Integer)
    add_eval!()
    λ = find_λ_AATP2(μ, s, C2)
    return sum(min.(max.(p0 .- λ .+ sum(max.(q0 .+ λ .- μ, 0))/C2, 0), C1)) - C2*μ
end


function solve_AATP2(p0::AbstractArray{<:Real},
                     q0::AbstractArray{<:Real},
                     C1::Real,
                     C2::Integer;
    ...
    if mean(partialsort(q0, 1:C2; rev = true)) + maximum(p0) <= 0
        ...
        g_μ(μ) = find_μ_AATP2(μ, s, p0, q0, C1, C2)
